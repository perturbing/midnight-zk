...
pub struct ConstraintSystem<F: Field> {
    ...
    // Whether the selectors of this constraint system were converted to fixed
    // columns via [Self::compress_selectors] (as opposed to
    // [Self::directly_convert_selectors_to_fixed]). Recorded so that proving
    // key generation and verifying key deserialization can replay the same
    // conversion.
    pub(crate) selectors_compressed: bool,
}
...
impl<F: Field> ConstraintSystem<F> {
    ...
    /// polynomials for those columns. Mutually-exclusive selectors whose gates
    /// have enough degree headroom share a single fixed column, whose value on
    /// each row picks out which of the combined selectors (if any) is active
    /// there.
    ///
    /// A shared (compressed) selector column holds values in `{0, ..., k}`
    /// (not just binary) and the substituted gate expressions gain
    /// indicator-polynomial factors, so no selector processed here qualifies
    /// for the simple-selector linearization optimization. All selector flags
    /// are marked non-simple, making these columns take the regular fixed
    /// column path: committed, opened and evaluated.
    ///
    /// Do not call this twice, nor mix it with
    /// [Self::directly_convert_selectors_to_fixed].
    pub fn compress_selectors(mut self, selectors: Vec<Vec<bool>>) -> (Self, Vec<Vec<F>>) {
        ...
        let selector_map = selector_map.into_iter().map(|a| a.unwrap()).collect::<Vec<_>>();
        let selector_replacements =
            selector_replacements.into_iter().map(|a| a.unwrap()).collect::<Vec<_>>();

        // Mark every selector as non-simple, so that provers and verifiers
        // treat the shared columns as regular fixed columns. Marking any of
        // them as simple would be unsound: the verifier would substitute 1 for
        // the column's evaluation, which no longer matches the multi-valued
        // column contents.
        for (flag, column) in self.selector_flags.iter_mut().zip(selector_map.iter()) {
            *flag = SelectorFlag(false, Some(column.index));
        }

        // Point each gate's queried selectors at the shared fixed column,
        // likewise marked non-simple so that the linearization argument does
        // not fold these columns into the linearization commitment.
        for gate in self.gates.iter_mut() {
            for s in &mut gate.queried_selectors {
                *s = Selector(selector_map[s.0].index, false);
            }
        }

        self.replace_selectors_with_fixed(&selector_replacements);
        self.num_selectors = 0;
        self.selectors_compressed = true;

        // Selector allocation is bounded by the pre-compression degree; the
        // substitution must not have increased the constraint-system degree.
